...
app.post('/api/projects', async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, message: 'Project created successfully', data: { ...projectData, _id: result.insertedId } });
  } catch (error) {
...
app.put('/api/projects/:id', async (req, res) => {
  try {
    ...
    if (result.matchedCount === 0) return res.status(404).json({ success: false, error: 'Project not found' });
...
app.delete('/api/projects/:id', async (req, res) => {
  try {
    ...
    if (result.deletedCount === 0) return res.status(404).json({ success: false, error: 'Project not found' });
    res.status(204).send();
  } catch (error) {
